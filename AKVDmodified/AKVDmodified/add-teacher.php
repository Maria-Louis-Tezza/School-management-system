...
if (isset($_POST['submit'])) {
    ...
    $sql = "SELECT COUNT(*) as count FROM teacher WHERE username = '$username'";
    ...
    // Sanitize the input username
    $empid = mysqli_real_escape_string($data, $_POST['empid']);

    // Query to check if the username already exists
    $sql = "SELECT COUNT(*) as count FROM teacher WHERE EmpID = '$empid'";
    ...
    if ($row['count'] > 0) {
        $errors['empid'] = 'Employee number already exists';
    }

    // validate emp id
    if (!preg_match("/^[0-9]{1,5}$/", $_POST['empid'])) {
        $errors['empid'] = 'EMP ID must be a number up to 5 digits long';
    }
    ...
    if (count($errors) > 0) {
        ...
        $sql = "INSERT INTO teacher(username, EmpId, image , email, password,lastaccessed) 
            VALUES ('$name', '$empid', '$dst_db', '$email', '$password','$usertype')";

        if (mysqli_query($data, $sql)) {
            $lastInsertId = mysqli_insert_id($data);
            if ($lastInsertId) {
                echo '<script>alert("Teacher info added successfully!"); window.location = "admin.php";</script>';
            } else {
                                                ...
                                                <div class="panel">
                                                    ...
                                                    <div class="panel-body">
                                                        ...
                                                        <form class="form-horizontal" method="post"
                                                            ...
                                                            <div class="form-group">
                                                                <label for="default" class="col-sm-2 control-label">Emp
                                                                    Id</label>
                                                                <div class="col-sm-10">
                                                                    <input type="text" name="empid" class="form-control"
                                                                        id="rollid" minlength="2" maxlength="5"
                                                                        required="required" autocomplete="off">
                                                                        <span class="help-block">Min 2 digits and Max 5 digits
                                                                    </span>
